...
import { getPaymentBaseReport } from "@/modules/reports/report-service";

const schema = z.object({ from: z.string(), to: z.string(), clientId: z.string().optional() });
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const report = await getPaymentBaseReport({
    ...
    return jsonError("No fue posible generar la base de pago.", 400, error);
  }
}
