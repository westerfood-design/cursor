...
export async function upsertMenuSelection(input: MenuSelectionInput) {
  ...
  if (!menuDay) {
    throw new Error("No se encontro el dia de menu solicitado.");
  }

  if (menuDay.menu.status !== MenuStatus.PUBLISHED) {
    throw new Error("El menu no esta publicado.");
  }
  ...
  if (!validMain || !validDessert) {
    throw new Error("La combinacion seleccionada no pertenece al menu del dia.");
  }
